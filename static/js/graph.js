...
// Unused functions moved to the bottom
function getEndpointPermissions(endpoint) {
    const parts = endpoint.split('.');
    let current = GRAPH_ENDPOINTS;
    ...
    if (typeof current === 'object' && current.delegatedPermission) {
        return [current.delegatedPermission, current.applicationPermission].filter(Boolean);
    }
